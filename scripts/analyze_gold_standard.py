...
class GoldStandardAnalyzer:
    ...
    def analyze_distribution(self):
        ...
        ticker_counts = [len(a.get('ticker_sentiments', {}))
        ...
        print(f"\n=== Mixed Sentiment Articles ===")
        print(
            f"Articles with mixed ticker sentiments: {mixed_sentiment} ({mixed_sentiment/len(self.annotations)*100:.1f}%)")

    def analyze_confidence(self):
        """Analyze confidence scores"""
        overall_confidences = [a.get('overall_confidence', 0)
                               for a in self.annotations]

        print("\n=== Confidence Analysis ===")
        print(
            f"Overall confidence - Mean: {np.mean(overall_confidences):.3f}, Std: {np.std(overall_confidences):.3f}")

        # Low confidence articles
        low_conf = [a for a in self.annotations if a.get(
            'overall_confidence', 0) < 0.6]
        ...
        if ticker_confidences:
            print(
                f"Ticker confidence - Mean: {np.mean(ticker_confidences):.3f}, Std: {np.std(ticker_confidences):.3f}")
    ...
    def find_interesting_cases(self):
        ...
        # 2. Low confidence
        low_conf = sorted(self.annotations, key=lambda x: x.get(
            'overall_confidence', 1))[:5]
        print(f"\n2. Lowest confidence articles:")
        for ann in low_conf:
            print(
                f"   - Confidence {ann['overall_confidence']:.2f}: {ann['title'][:60]}...")
    ...
    def validate_consistency(self):
        ...
        for ann in self.annotations:
            ...
            ticker_sentiments = [info['sentiment']
    ...
    def create_visualizations(self, output_dir: Path):
        ...
        overall_conf = [a.get('overall_confidence', 0)
                        for a in self.annotations]
        ax1.hist(overall_conf, bins=20, alpha=0.7,
        ...
        if ticker_conf:
            ax2.hist(ticker_conf, bins=20, alpha=0.7,
...
def main():
    ...
    parser.add_argument('--input', type=str,
                        default='data/gold_standard_annotations.jsonl')
    parser.add_argument('--output-dir', type=str, default='data/plots')
    parser.add_argument('--export', type=str,
                        default='data/gold_standard_eval.jsonl')
