...
def analyze_results(results: List[Dict], name: str) -> Dict:
    ...
    if stats['total_articles'] > 0:
        stats['avg_tickers_per_article'] = total_tickers / stats['total_articles']
    ...
    stats['ticker_coverage_pct'] = (stats['articles_with_tickers'] / total * 100) if total > 0 else 0
...
def create_comparison_plots(stats_list: List[Dict], output_dir: str = "data/plots"):
    ...
    for i, sentiment in enumerate(sentiments):
        values = [stats['sentiment_pct'].get(sentiment, 0) for stats in stats_list]
        offset = (i - 1) * width
        bars = ax.bar([x + offset for x in x_pos], values, width, label=sentiment)
    ...
    ax.set_title('Sentiment Distribution Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x_pos)
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/sentiment_distribution_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    bars = ax.bar(names, ticker_coverage, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/ticker_coverage_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3. Neutral Bias Reduction Chart
    fig, ax = plt.subplots(figsize=(8, 6))
    
    neutral_percentages = [stats['sentiment_pct'].get('Neutral', 0) for stats in stats_list]
    colors = ['red' if pct > 60 else 'orange' if pct > 40 else 'green' for pct in neutral_percentages]
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/neutral_bias_reduction.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def generate_comparison_report(stats_list: List[Dict]) -> str:
    ...
    for i, stats in enumerate(stats_list):
        ...
        report.append(f"Articles with tickers: {stats['articles_with_tickers']} ({stats['ticker_coverage_pct']:.1f}%)")
        ...
        for sentiment in ['Positive', 'Neutral', 'Negative']:
            ...
            if i > 0 and sentiment == 'Neutral':
                ...
                if improvement > 0:
                    report.append(f"    → Improvement: {improvement:.1f} percentage points reduction")
    ...
    neutral_scores = [(stats['name'], stats['sentiment_pct'].get('Neutral', 0)) for stats in stats_list]
    best_method = min(neutral_scores, key=lambda x: x[1])
    
    report.append(f"Best neutral bias reduction: {best_method[0]} ({best_method[1]:.1f}% neutral)")
    ...
    for stats in stats_list[1:]:
        ...
        if improvement > 0:
            report.append(f"Ticker coverage improvement ({stats['name']}): +{improvement:.1f} percentage points")
...
def main():
    ...
    result_files = [
        ...
        ("data/processed_articles_enhanced.jsonl", "Enhanced Pipeline (conf_weighted)"),
    ...
    for filepath, name in result_files:
        if os.path.exists(filepath):
            ...
            print(f"  Neutral percentage: {stats['sentiment_pct'].get('Neutral', 0):.1f}%")
