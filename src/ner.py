...
EXCHANGE_PATTERN = re.compile(r"\b([A-Z]{2,5})(?:\s+(?:on|traded\s+on|listed\s+on)\s+(?:NYSE|NASDAQ|the\s+exchange))", re.IGNORECASE)
STOCK_MENTION_PATTERN = re.compile(r"\b([A-Z]{2,5})\s+(?:stock|shares|equity|securities)", re.IGNORECASE)
PRICE_PATTERN = re.compile(r"\$([A-Z]{2,5})\b")
FINANCIAL_ACTION_PATTERN = re.compile(r"(?:buy|sell|hold|upgrade|downgrade|target|rating)\s+([A-Z]{2,5})\b", re.IGNORECASE)

# Exchange suffixes to clean
EXCHANGE_SUFFIXES = {'.US', '.TO', '.L', '.PA', '.F', '.HM', '.MI', '.AS', '.MX', '.SA', '.BE', '.DU', '.MU', '.STU', '.XETRA'}
...
class EnhancedNER:
    ...
    def _load_symbol_list(self, csv_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            print(f"❌ Warning: {csv_path} not found. Creating empty symbol dictionary.")
            return {}
