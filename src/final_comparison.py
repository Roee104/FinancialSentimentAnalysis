...
"""
Final comparison analysis using the recent results from pipeline_optimized_fixed
"""
...
def create_baseline_comparison():
    ...
    recent_enhanced = analyze_recent_results('data/processed_articles.jsonl', last_n=5000)
    ...
    print(f"   Average tickers per article: {recent_enhanced['avg_tickers']:.2f}")
    print(f"   Articles with tickers: {recent_enhanced['articles_with_tickers']/recent_enhanced['total']*100:.1f}%")
    ...
    ax1.pie(sizes, labels=sentiments, colors=colors, autopct='%1.1f%%', startangle=90)
    ax1.set_title('Enhanced Pipeline Sentiment Distribution\n(Recent 5000 Articles)', fontsize=14)
    
    # Comparison with theoretical baselines
    methods = ['Original\n(80% Neutral)', 'Enhanced Pipeline\n(Recent)', 'Target\n(Balanced)']
    neutral_values = [80, recent_enhanced['sentiments'].get('Neutral', 0)/recent_enhanced['total']*100, 33.3]
    
    bars = ax2.bar(methods, neutral_values, color=['red', 'green', 'blue'], alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig('data/plots/final_sentiment_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    enhanced = [
        recent_enhanced['sentiments'].get('Neutral', 0)/recent_enhanced['total']*100,
        recent_enhanced['articles_with_tickers']/recent_enhanced['total']*100,
        100 - abs(33.3 - recent_enhanced['sentiments'].get('Positive', 0)/recent_enhanced['total']*100)
    ]
    
    x = np.arange(len(metrics))
    width = 0.35
    
    bars1 = ax.bar(x - width/2, original, width, label='Original Pipeline', color='red', alpha=0.7)
    bars2 = ax.bar(x + width/2, enhanced, width, label='Enhanced Pipeline', color='green', alpha=0.7)
    
    ax.set_xlabel('Metrics', fontsize=12)
    ax.set_ylabel('Percentage (%)', fontsize=12)
    ax.set_title('Pipeline Improvement Summary', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('data/plots/improvement_summary.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("✅ Saved improvement summary plot")
    
    # 3. Generate final report
    neutral_pct = recent_enhanced['sentiments'].get('Neutral', 0)/recent_enhanced['total']*100
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("⚠️  Could not apply NER patch")
