...
def main():
    ...
    experiments = [
        # 1. Run main pipeline (baseline with default settings)
        (
            "python src/main_pipeline.py",
            "Running MAIN pipeline (baseline)"
        ),
        ...
        # 3. Run enhanced pipeline with optimized settings
        (
            "python src/main_pipeline.py --output data/processed_articles_enhanced.jsonl",
            "Running ENHANCED pipeline (conf_weighted, threshold=0.1)"
        ),

        # 4. Run enhanced pipeline with different threshold
        (
            "python src/main_pipeline.py --threshold 0.15 --output data/processed_articles_enhanced_t15.jsonl",
            "Running ENHANCED pipeline (conf_weighted, threshold=0.15)"
        ),

        # 5. Run enhanced pipeline with majority method
        (
            "python src/main_pipeline.py --method majority --output data/processed_articles_majority.jsonl",
